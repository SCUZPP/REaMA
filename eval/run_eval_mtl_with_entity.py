...
def main(args):
    ...
    for index, example in enumerate(test_data):
        ...
        content = example["content"].strip().split("[SEP]")
        task_intro = content[0].strip()
        content = content[-1].strip()
        if "cdr" in args.data_dir: 
            prompt_discribe = "None-CID, CID"
            cloze = "The relation between " + task_intro.replace("What is [CID] between ", "").replace("@/ChemicalSrc$", "").replace("@/DiseaseTgt$", "").replace("@ChemicalSrc$", "").replace("@DiseaseTgt$", "").replace("  ", " ").replace("?", "").strip() + " is"

        elif "biorex" in args.data_dir: 
            prompt_discribe = "None, Association, Bind, Comparison, Conversion, Cotreatment, Drug_Interaction, Negative_Correlation, Positive_Correlation"
        elif "ddi" in args.data_dir: 
            prompt_discribe = "None-DDI, effect, advise, mechanism, int"
            cloze = "The relation between " + task_intro.replace("What is [DDI] between ", "").replace("@drugSrc$", "").replace("@/drugSrc$", "").replace("@drugTgt$", "").replace("@/drugTgt$", "").replace("  ", " ").replace("?", "").strip() + " is"

        elif "drugprot" in args.data_dir: 
            prompt_discribe = "None-BC7, PRODUCT-OF, INDIRECT-UPREGULATOR, ACTIVATOR, ANTAGONIST, INDIRECT-DOWNREGULATOR, SUBSTRATE, DIRECT-REGULATOR, PART-OF, INHIBITOR, AGONIST-ACTIVATOR, SUBSTRATE_PRODUCT-OF, AGONIST, AGONIST-INHIBITOR"
            cloze = "The relation between " + task_intro.replace("What is [Drugprot] between ", "").replace("@CHEMICALSrc$", "").replace("@/CHEMICALSrc$", "").replace("@GENETgt$", "").replace("@/GENETgt$", "").replace("  ", " ").replace("?", "").strip() + " is"

        elif "disgenet" in args.data_dir: 
            prompt_discribe = "None-dis, dis-Association"
            cloze = "The relation between " + task_intro.replace("What is [DISGENET] between ", "").replace("@DiseaseSrc$", "").replace("@/DiseaseSrc$ ", "").replace("@GeneTgt$", "").replace("@/GeneTgt$", "").replace("  ", " ").replace("?", "").strip() + " is"

        elif "nary" in args.data_dir:
            content = example["content"]
            type1 = example['type1']
            type2 = example['type2']
            prompt_discribe = "None-nary, resistance, response, resistance or non-response, sensitivity"
            cloze = "The relation between " + type1 + " and " + type2 + " is"

        elif "drug_gene_var" in args.data_dir:
            content = example["content"]
            type1 = example['type1']
            type2 = example['type2']
            type3 = example['type3']
            prompt_discribe = "None-nary, resistance, response, resistance or non-response, sensitivity"
            cloze = "The relation between " + type1 + ", " + type2 + ", and " + type3 + " is"
            
        
        prompt_prefix = "The following is a text annotated with two entities from the biomedical literature. Please select the one from '%s' that correctly describe the relationship between the two entities.\n\n" % prompt_discribe
        if "drug_gene_var" in args.data_dir:
            prompt_prefix = "The following is a text annotated with three entities from the biomedical literature. Please select the one from '%s' that correctly describe the relationship between the three entities.\n\n" % prompt_discribe            
    
    
        if args.use_chat_format:
            prompt = "<|user|>\n" + prompt_prefix + "Input: " + content + "\n"+ cloze + "\nAnswer:" + "\n<|assistant|>\n" 
        else:
            prompt = prompt_prefix + "Input: " + content + "\n"+ cloze +"\nAnswer:"
